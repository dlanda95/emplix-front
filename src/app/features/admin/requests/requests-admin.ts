...
import {
  AdminPageLayout, Avatar, Badge, Banner, Button, DataTable, EmptyState,
  FilterChips, LoadingSkeleton, Modal, PageHeader, Pagination, SectionCard, AppInput,
} from '@shared/ui';
...
const FIELD_LABELS: Record<string, string> = {
  firstName: 'Primer Nombre', middleName: 'Segundo Nombre',
  lastName: 'Apellido Paterno', secondLastName: 'Apellido Materno',
  birthDate: 'Fecha de Nacimiento', gender: 'Género',
  maritalStatus: 'Estado Civil', nationality: 'Nacionalidad',
  academicLevel: 'Grado Académico',
  birthCountry: 'País de Nacimiento', birthRegion: 'Departamento de Nacimiento',
  birthDistrict: 'Distrito de Nacimiento',
  documentType: 'Tipo de Documento', documentId: 'N° Documento',
  licenseNumber: 'Brevete', docAddress: 'Dirección (Doc. Identidad)',
  docDistrict: 'Distrito (Doc. Identidad)', docDepartment: 'Departamento (Doc. Id.)',
  ...
  afpType: 'Sistema AFP', afpEntity: 'Entidad AFP', afpCommission: 'Tipo de Comisión',
  bankEntity: 'Entidad Bancaria', bankAccount: 'N° Cuenta', bankCci: 'CCI',
};

const FIELD_OPTIONS: Record<string, Array<{ value: string; label: string }>> = {
  gender: GENDER_OPTIONS, maritalStatus: MARITAL_STATUS_OPTIONS,
  academicLevel: ACADEMIC_LEVEL_OPTIONS, documentType: DOCUMENT_TYPE_OPTIONS,
  afpType: AFP_TYPE_OPTIONS, afpEntity: AFP_ENTITY_OPTIONS, afpCommission: AFP_COMMISSION_OPTIONS,
};
...
function buildDiff(data: Record<string, unknown> | null | undefined): Array<{ field: string; label: string; old: string; new: string }> {
  ...
  return Object.keys(data)
    ...
    .filter(k => norm(previous[k]) !== norm(data[k]))
    .map(k => ({ field: k, label: FIELD_LABELS[k], old: display(previous[k], k), new: display(data[k], k) }));
}
...
export class RequestsAdmin implements OnInit {
  ...
  // Modal de detalle (desktop)
  readonly detailRequest = signal<ChangeRequest | null>(null);
  ...
  private loadAll(): void {
    ...
    this.svc.getAll({
      page: this.page(), limit: PAGE_SIZE,
      status, search: this.searchCtrl.value ?? '',
      ...
      error: err => {
        this.isLoading.set(false);
        this.loadError.set(err?.error?.message ?? 'Error al cargar las solicitudes.');
      },
    });
  }
  ...
  // Modal de detalle
  openDetail(req: ChangeRequest): void  { this.detailRequest.set(req); }
  closeDetail(): void                   { this.detailRequest.set(null); }
  ...
  private updateStatus(id: string, status: RequestStatus): void {
    ...
    if (detail?.id === id) this.detailRequest.set({ ...detail, status });
  }
  ...
  diffCount(r: ChangeRequest): number {
    return buildDiff(r.data).length;
  }
}
